...
- **Production bootstrap is deliberately NOT `prisma/seed.js`**: `start.js` only seeds when the DB is completely empty, and when it does, it now runs **`prisma/seed-prod.js`**, not `seed.js`. `seed.js` creates 2 demo camps/11 rooms/5 accounts all sharing the password `Password123!` — a password printed in this repo's own README/CHECKPOINT, which would be a live credential in production if it ever ran there. `seed-prod.js` creates only the checklist template (via the now-shared `prisma/checklistData.js`, used by both seeders so the real questions live in one place) plus a single admin account.
- **Three-layer admin-credential safety net** (added after the user asked "what's the worst case if a human forgets to grab the one-time password" — the original design had no recovery path at all): shared logic lives in **`prisma/adminBootstrap.js`**'s `createOrResetAdmin(prisma, { label, mode })`, used by both `seed-prod.js` (`mode: 'create'`, only runs when zero users exist) and the new **`prisma/reset-admin-password.js`** (`mode: 'reset'`, runnable anytime).
  1. **`ADMIN_PASSWORD` env var, if set** — deployer's own chosen password, hashed and used directly, `mustChangePassword: false` (they already know it, no forced-change friction). Validated to be at least 8 chars — `resolveCredentials()` throws and refuses to boot if it's shorter, rather than silently accepting a weak password. `ADMIN_EMAIL` still applies (defaults `admin@checker.local`).
  2. **Not set → falls back to `generateTempPassword()`** (same helper `adminUsers.js`'s `generate-credentials` endpoint uses), printed once to stdout, `mustChangePassword: true`.
  3. **Recovery**: `node prisma/reset-admin-password.js`, run anytime via the platform's shell/exec — finds the admin by email and resets their password (also reactivates `isActive` and re-adds the ADMIN role assignment if somehow missing), or creates one fresh if none exists. Does not touch any other data. Same `ADMIN_PASSWORD`/`ADMIN_EMAIL` env vars apply.
  Verified all five paths against throwaway scratch SQLite DBs: first-boot random-password, first-boot explicit `ADMIN_PASSWORD` (confirmed login works, `mustChangePassword: false`, password never printed), recovery-reset on an existing admin (both random and explicit), too-short `ADMIN_PASSWORD` correctly refused, and recovery-create when no admin exists at all.
...
- **Never point `scripts/start.js`'s first-boot seed at `prisma/seed.js`** — that seeder's password (`Password123!`) is public in this repo's own docs; production must keep using `prisma/seed-prod.js` (checklist template + one admin)
- Keep `prisma/seed-prod.js` and `prisma/reset-admin-password.js` sharing `adminBootstrap.js`'s `createOrResetAdmin` — don't fork the credential logic between them, that's exactly the kind of drift that reintroduces the "no recovery path" problem this was built to fix
- The `ADMIN_PASSWORD` minimum-length check in `adminBootstrap.js` (`resolveCredentials`) must keep failing loudly (refuse to boot) rather than silently accepting a weak explicit password
